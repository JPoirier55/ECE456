...
def main(argv):
    ...
    input_message = read_file('input.txt')
    bits_encrypted = encrypt(input_message)
    text_encrypted = frombits(bits_encrypted)
    write_file('output.txt', text_encrypted)

    output_message = read_file('output.txt')

    print frombits(decrypt(output_message))
